# docker-ubuntu-vnc-desktop with cuda
## What is this
...
サーバー上で以下のコマンドを実行
```
$ ARCH=amd64 IMAGE=nvidia/cuda:11.1.1-cudnn8-devel-ubuntu20.04 make build
...
ローカルのブラウザーで`<サーバーのアドレス>:6080`を開き、ユーザーを`ubuntu`、パスワードを`password`にするこでアクセスできる。
...
現在の仕様ではglxgearsやDonkey CarはGPUを利用できない。（誰かpull request出してくれると助かる）

以下オリジナルのdocker-ubuntu-vnc-desktopのREADME
